...
5. **输出 Product-Spec.md：** 读取 `<skill-path>/templates/product-spec-template.md` 作为结构参考，同时读取 `<skill-path>/templates/product-spec-example.md` 作为填写粒度参考。逐章节填充，保存到项目根目录。各章节填写要点：
   ...
   如果某章节的内容需要超出 PRD 或对话范围的推断，用 `[待确认]` 标注，而不是用假设内容填充。
...
5. **更新文档：** 把检查结果写回 Product-Spec.md 的功能清单（更新方括号状态）。向用户展示拟变更内容并确认后再写入。
6. **更新计划：** 如果实现计划目录下有计划文件，同步标注已完成的任务。生成报告后，注明因搜索限制无法验证的功能，并将其标记为"覆盖率不确定"而非"未实现"。
...
4. **更新 Product-Spec.md：** 把变更同步到功能清单和相关章节。向用户展示拟变更内容并确认后再写入。列出本次同步中审阅的所有设计文档文件。
